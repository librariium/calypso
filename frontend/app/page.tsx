...
interface Book {
  ...
  thumbnail: string;
  score: number;
  // 👇 NEW: Added these fields for the new data
  rating?: number;
  readers?: number;
}
...
export default function Home() {
  ...
  // ---------------------------------------------------------
  // 🎨 UI RENDER (Pastel Edition + Pro Data)
  // ---------------------------------------------------------
  return (
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-3 flex-wrap">
                      ...
                      {book.rating && book.rating > 0 && (
                        <span className="bg-amber-50 text-amber-500 text-xs font-bold px-3 py-1 rounded-full border border-amber-100 flex items-center gap-1">
                          ⭐ {book.rating.toFixed(1)}
                        </span>
                      )}
                    </div>
                    
                    <h3 className="text-xl font-bold text-slate-800 leading-tight mb-1 group-hover:text-rose-500 transition-colors">
                      {book.title}
                    </h3>
                    <p className="text-sm text-slate-400 font-medium mb-2">{book.authors}</p>
                    
                    {/* 🔥 NEW: Trending Badge */}
                    {book.readers && book.readers > 10000 && (
                        <div className="flex items-center gap-1 text-xs text-orange-400 font-bold mb-4">
                            <span>🔥 Trending ({book.readers.toLocaleString()} readers)</span>
                        </div>
                    )}
